...
/** Order = priority (first match wins). Keep most specific traps first. */
const PATTERNS = [
  {
    id: "resilience",
...
function blobFor(q, ex, sig, { includeDistractors = false } = {}) {
  // Stem + correct answers + solution why only by default.
  // Exclude Process names from explanations ("Develop Team" → fake competency).
  const parts = [
    q?.text,
    q?.correctLabel,
...
function assign(q, ex, sig) {
  ...
  // Second pass: allow distractors only when correct-only blob had no match.
  // Recovers stems whose PMI cue lives in wrong-option contrast language.
