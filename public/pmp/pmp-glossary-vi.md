...
> Từ điển nghĩa từ exam English → VI (PMBOK 8).
...
| Cụm | Ý | PMP |
|-----|---|-----|
| **Context cues** | Ngữ cảnh — im lặng, thứ bậc, gián tiếp | Cultural awareness |
...
| Adapt comms + **compliance** | Chỉ informal meetings khi cần compliance |
| Change request dù sponsor pressure | Add scope informally *for rapport* |
...
| Signal | Đáp |
|--------|-----|
| Executives concise · teams task-level detail | **Tailor** artifacts by group — consistent data → A |
| Reports too technical · business questions | **Update comms plan** — tailor format → C |
...
| Brief mention of new risks | **Document & analyze** ngay → C |
...
| Signal | Đáp |
|--------|-----|
| Conflicts about design | **Facilitate** team discussions → D |
| Global multidisciplinary + culture | **Culturally responsive communication protocol** → C |
| Interdisciplinary + tight deadline | **MVP** → C |
...
**Đáp án đúng:** Cost/staffing **tăng** khi work progresses → **giảm** gần completion · Risk/uncertainty **giảm over time**
...
**Hay gặp:** Differing site conditions · spec không rõ, đòi remediation · payment/penalty dispute

**Thang leo thang:** Negotiation → Mediation → Arbitration → DRB → Litigation (last resort)

**PM làm gì:**
1. Đọc contract — escalation & claims procedures
2. Document records (hỗ trợ claim, không thay resolve)
3. **Negotiate** khi resolve nhanh, no external parties
4. Impact analysis trước enforce penalty
...
**Remediation (gaps):** Skill gap · Quality noncompliance · Compliance/security · Governance weakness

**PM làm gì:**
| Signal | Đáp |
|--------|-----|
| Apply remedies / penalties | **Structured performance review** trước → D |
| Vendor noncompliance | **Impact analysis FIRST** → A |
...
| **Pattern** | Stakeholders → analysis · Defects → RCA · Changes → baseline triage |
...
| **PMP** | *intermittent errors* trong pilot → pause + root cause |

### Concessions
| | |
|---|---|
| **Nghĩa** | Nhượng bộ trong đàm phán — bớt yêu cầu để giảm conflict |
| **PMP** | *minor concessions* thường **trap** khi requirement **integral to contract** ( enforce durability) |
...
| **PMP** | Reframe: **standards met** + **improve future performance** (,E) |
...
| **PMP** | Post-acquisition → engagement **aligned to acquiring company culture** |

### Proportionate
| | |
|---|---|
| **Nghĩa** | Tương xứng với **mức impact** — right-sized response |
| **PMP** | Workaround *hoặc* structural change **as needed** |
| **≠ Proportion** | *Proportion* = tỷ lệ/share |
...
| **PMP** | *pinpointed vagueness in policies* → clarify/revisit **đúng** policy |
| **≠** | Suspected / possible |
...
| **PMP** | Change *must be justified by measurable benefits* → quantify value |
...
| **PMP** | *reduce ramp-up time* → institutionalize via **PMO/OPA** · *before execution ramps up* → working agreements · PM *ramping up* → engage stakeholders + action plan |
...
| **PMP** | *departmental shortfalls* ← divert project funds = governance violation · *budget shortfall* → escalate steering |
...
| **PMP** | *labor … susceptible to strikes* = **risk driver** → identify & register (,D) |
...
| **PMP** | *extensive recoding … to **retrofit** the software* = scope/effort lớn để tương thích tech mới |
...
| **PMP** | Incorporate resilience into PM approach · long-term org resilience |
...
| **Agile** | Readiness criteria / DoR |
| **Closing** | Closure / operational criteria · confirm readiness |
| **OCM** | Change readiness assessment → gap / interventions |

### Alleges
| | |
|---|---|
| **Nghĩa** | Cáo buộc / claim chưa chứng minh |
| **PMP** | *alleges poor accountability* → clarify WBS dictionary / ownership |
...
| | |
|---|---|
| **Plant turnaround** | Đợt bảo dưỡng/dừng máy — FFP nếu scope rõ; sớm hơn → CR |
| **Tight turnaround** | Deadline gấp → conflict action plan |

### Drivers
| | |
|---|---|
| **Nghĩa** | Nhân tố / lực đẩy gây ra kết quả |
| **Cost / variance** | Forecast + variance analysis với drivers & impacts |
| **Business / compliance** | Traceability matrix |
| **Systemic** | RCA trước corrective action |
| **Literal** | *rural drivers* = tài xế |
...
| **Backlog** | Elaborate trước sprint |
| **Progressive** | Hybrid / rolling wave |
